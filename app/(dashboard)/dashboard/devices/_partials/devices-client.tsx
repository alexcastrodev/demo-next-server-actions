...
import type { Device, GetDevicesResponse } from "core/entities";
import { DataTable } from "ui/datatable";

interface DevicesClientProps {
  initialData: GetDevicesResponse;
}

export function DevicesClient({ initialData }: DevicesClientProps) {
  const { t } = useTranslation();
  const router = useRouter();
  const { data, isFetching } = useGetDevices({ initialData });
